...
from lambdas.tle_fetcher.handler import (
    archive_raw_tle,
    handler,
    parse_tle,
    write_satellites,
...
def test_handler_end_to_end(monkeypatch):
    ...
    with patch(
        "lambdas.tle_fetcher.handler.fetch_tle_text", return_value=SAMPLE_TLE
    ):
...
def test_handler_raises_on_empty_tle_response(monkeypatch):
    ...
    with patch("lambdas.tle_fetcher.handler.fetch_tle_text", return_value=""):
